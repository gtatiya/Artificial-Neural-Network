...
"""
The last step is to study the attention_decoder itself.
As the name suggests, the main feature of this decoder is that it computes a set of attention weights over the hidden states that the encoder emitted during encoding.
After defensive checks, we reshape the hidden features to the right size:
"""
def attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    """
    We now define the attention() method itself, which consumes a query vector and returns the attention-weighted vector over the hidden states.
    This method implements the same attention as described in the previous section:
    """
    def attention(query):
    ...
    """
    Now we loop over the rest of the inputs.
    We perform a defensive check to ensure that the input at the current time step is the right size.
    Then we run the RNN cell as well as the attention query.
    These two are then combined and passed to the output according to the same dynamics:
    """
...
"""
The embedding_attention_decoder is a simple improvement over the attention_decoder described in the previous section; essentially, the inputs are projected to a learned embedding space, which usually improves performance.
The loop function, which simply describes the dynamics of the recurrent cell with embedding, is invoked in this step:
"""
...
"""
When initializing this model, several flags and arguments are passed as function arguments.
One argument of particular note is feed_previous. When this is true, the decoder will use the outputted logit at time step T as input to time step T+1. In this way, it is sequentially decoding the next token based on all tokens thus far.
We can describe this type of decoding, where the next output depends on all previous outputs, as autoregressive decoding:
"""
